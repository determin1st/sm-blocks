...
"use strict";
var smCatalogueKing, slice$ = [].slice, arrayFrom$ = Array.from || function(x){return slice$.call(x);};
smCatalogueKing = async function(){
  var BRAND, soFetch, newPromise, delay, querySelectorChildren, querySelectorChild, newMetaObject, BlockState, sMainSection, sCart, KING, mProductsGrid, mCategoryFilter, mPriceFilter, mPaginator, mOrderer;
  BRAND = 'sm-blocks';
  ...
  KING = function(){
    ...
    gridLoader = function(){
      var cooldown, iFetch, res, req, setState, unloadItems, newMasterPromise;
      cooldown = 800;
      ...
      return async function(){
        ...
        for (i$ = 0, len$ = (ref$ = gridControl).length; i$ < len$; ++i$) {
          ...
          if (c.level < gridState.level) {
            c.event('lock');
          } else if (!c.event('change', gridState)) {
            return true;
          }
        }
        ...
        if ((b = (await a.readJSON())) === null || gridState.dirty) {
          a.cancel();
          return gridState.dirty;
        }
        gridState.total = c = b.total;
        gridState.count = (d = c - req.offset) < gridList.length
          ? d
          : gridList.length;
